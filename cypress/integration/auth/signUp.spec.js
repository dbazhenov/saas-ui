...
context('Sign Up', () => {
  ...
  it('SAAS-T78 - Verify Sign Up on Percona Portal', () => {
    ...
    cy.mailosaurGetMessage(mailosaurServerId, { sentTo: newUser.email }, { timeout: 20000 }).then(
      (message) => {
        expect(message.text.body)
          .to.contain(`Hi ${newUser.firstName},`)
        ...
        expect(message.metadata.headers.find(({ field }) => field === 'Subject').value).to.be.equal(
          signUpPage.constants.labels.activateEmailSubject,
        );
        const activateLink = message.html.links.find(
          ({ text }) => text.trim() === signUpPage.constants.labels.activateAccount,
        ).href;
  ...
  it('SAAS-T85 - Verify Sign Up if user already has Percona account', () => {
    ...
    cy.get(signUpPage.locators.termsOfService).check({ force: true });
    cy.get(signUpPage.locators.registerButton).isEnabled().click();
    cy.get(signUpPage.locators.registrationAlert).hasText(
      signUpPage.constants.messages.emailAlreadyRegistered,
    );
    cy.cleanUpAfterTest([newUser]);
  });
  ...
  it('SAAS-T113 - Verify Sign up redirects to Okta widget', () => {
    ...
    cy.findByTestId(signUpPage.locators.privacyPolicyLink).hasAttr(
      'href',
      signUpPage.constants.links.platformPrivacy,
    );
    cy.get(signUpPage.locators.termsOfService).check({ force: true });
